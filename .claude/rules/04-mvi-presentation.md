...
- Compose (`uiFramework` compose): `*Screen` collects with `collectAsStateWithLifecycle` + `LaunchedEffect` for effects; private `*ScreenContent` renders; no `NavController` in the feature (`28-compose-ui`)
