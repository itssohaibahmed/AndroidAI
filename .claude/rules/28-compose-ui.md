...
# Compose UI (invariants)

**Full detail (modules, naming, NavGraph, Screen template):** [reference/compose-ui.md](reference/compose-ui.md)
...
- One Gradle module per feature: `:feature-<kebab>` — **no** monolith `:presentation`
- Package: `{applicationId}.feature.<name>` with `di` / `intent` / `state` / `effect` / `viewModel` + `*Screen.kt` at the feature root
- `*Screen` (stateful: `koinViewModel`, `collectAsStateWithLifecycle`, `LaunchedEffect` for effects) + private `*ScreenContent` (stateless)
...
Read [reference/compose-ui.md](reference/compose-ui.md) when scaffolding or changing Compose screens.
