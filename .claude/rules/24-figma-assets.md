...
Figma → Android XML work: `download_assets`, `get_design_context`, or implementing a Figma screen/link into layouts/drawables.
...
- Always `ShapeableImageView` (`siv`) for **display-only** images — never plain `ImageView`
- Clickable icons / toolbar actions → `MaterialButton` + `ButtonStyle.IconButton` (`app:icon`) — not clickable `ShapeableImageView`
- Solid + stroke button surfaces from Figma → `app:backgroundTint` / `app:strokeColor` / `app:strokeWidth` / `app:cornerRadius` on the button — **do not** download or author a `bg_shape_*` oval/rect for that
- Bind images in code with `siv.loadImage(...)` (Glide / `ImageViewExtensions`) — not `setImageResource` for list/remote/dynamic assets
- `android:contentDescription="@string/cd_*"` (Content Descriptions section in `strings.xml`)
