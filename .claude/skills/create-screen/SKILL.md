...
Follow `.claude/rules/` — especially `00-global`, `01-feature-checklist`, and whatever each leaf skill lists.

Obey `.claude/project-settings.json` when present (`uiFramework`, `writeTestsWithFeatures`, `orientation`, `themeModes`).
...
|---------------|-----------|
| `xml` (default) | **`figma-to-xml`** (`.claude/skills/figma-to-xml/SKILL.md`) |
| `compose` | **`figma-to-compose`** (`.claude/skills/figma-to-compose/SKILL.md`) |
...
→ Run **`create-clean-architecture`** (`.claude/skills/create-clean-architecture/SKILL.md`)

Skip this step when only existing domain APIs are reused.

### 3 — Presentation MVI

→ Run **`create-mvi`** (`.claude/skills/create-mvi/SKILL.md`)
